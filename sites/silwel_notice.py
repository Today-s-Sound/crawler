...
class SilwelNoticeCrawler(SiteCrawler):
    ...
    def fetch_post_content(self, post_url: str) -> str:
        ...
        if not content:
            # "공지사항" 텍스트가 포함된 영역 근처의 테이블 찾기
            all_tables = soup.find_all("table")
            for table in all_tables:
                # 테이블 앞뒤 텍스트에서 "공지사항" 키워드 확인
                prev_text = table.find_previous(string=True)
                if prev_text and "공지사항" in str(prev_text):
                    # 테이블 내부에 충분한 텍스트가 있는지 확인 (본문일 가능성)
                    table_text = table.get_text(strip=True)
                    if len(table_text) > 100:  # 본문은 보통 100자 이상
                        content = table
                        break
